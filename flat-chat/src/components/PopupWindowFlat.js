...
const PopupWindowFlat = ({ data, onClose, companyName }) => {
  ...
  useEffect(() => {
    ...
    const url = base_url + '/api/flat/query?latitude=' + data[0] + '&longitude='+data[1];
    ...
    <div className="popup-window absolute left-2 top-20 flex flex-col w-11/12 h-5/6 md:w-1/3  border border-green-200 border-1 rounded-lg bg-gradient-to-br from-zinc-700 via-zinc-800 to-zinc-900 opacity-75" style={{ zIndex: 1000 }}>
      <div className="p-4 flex justify-between ">
        <div className="flex justify-inline">
          <div className="h-9 w-9"><img src={companyLogo} alt="Company Logo" /></div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="grid grid-cols-1" style={{ zIndex: 4000 }}>
          {apiResult && (
            <div key={apiResult._id} className=" p-1 m-2 ">
              <div className="flex justify-between content-between">
                <div className=" flex text-sm text-white ">
                  <div className="m-0.5"></div>
                  <div className="m-1">Address : {apiResult.Address}</div> 
                </div>
              </div>

              <div className="flex justify-between content-between items-baseline px-1">
                <div className="flex text-sm text-zinc-400">
                  <div className="my-0.5 mx-1">Bedrooms: {apiResult.bedrooms}</div>
                </div>
                <div className="">
                  <div className="text-xm text-zinc-400 italic">Rent: {apiResult.rent}</div>
                </div>
              </div>
              <div className="flex justify-between my-1">
                <div className="text-xs px-2 text-zinc-300 w-full description">{apiResult.description}
                </div>
              </div>
              <div className="m-2">
                  <Button
                  ...
                  >
                    <a href={`mailto:${apiResult.createdBy}`}>Gmail</a>
                  </Button>
                </div>
            </div>
          )}
...
export default PopupWindowFlat;
